...
H = 0.95 # Hurst parameter for fGBM
...
results_comparison.to_csv('option_pricing_comparison_H_0_95.csv', index=False)

# Plotting the comparison
plt.figure(figsize=(10, 6))
plt.plot(results_comparison['Volatility'], results_comparison['GBM Call Price'], label='GBM Call Price', marker='o', linestyle='-')
plt.plot(results_comparison['Volatility'], results_comparison['fGBM Call Price'], label='fGBM Call Price (H=0.95)', marker='x', linestyle='--')
plt.xlabel('Volatility')
plt.ylabel('Call Option Price')
plt.title('Comparison of GBM and fGBM Call Option Prices (H=0.95)')
plt.legend()
plt.grid(True)
plt.savefig('option_prices_comparison_H_0_95.png')
plt.show()
